...
df_tt <- df %>%
  ...
  mutate(
    ...
    # -------- NEW: Relative error (%) on the YEARS scale --------
    Log_Diff = Pred_ln_years - Known_ln_years,
    Rel_Error_pct_years = 100 * abs(exp(Log_Diff) - 1),
...
errors_out <- df_tt %>%
  ...
  dplyr::arrange(Dataset, dplyr::desc(Abs_Error_years))
...
summary_tbl <- df_tt %>%
  ...
  summarise(
    ...
    Mean_Rel_Error_pct_years   = mean(Rel_Error_pct_years, na.rm = TRUE),
    Median_Rel_Error_pct_years = median(Rel_Error_pct_years, na.rm = TRUE),
    .groups = "drop"
  )
...
# Relative error stats (means only) — NEW metric
means_rel <- df_tt %>%
  group_by(Dataset) %>%
  summarise(mean_rel = mean(Rel_Error_pct_years, na.rm = TRUE), .groups = "drop")

# 2) Absolute Error plot (violin)
...
# 3) Relative Error plot (violin, YEARS %)
#    Auto-cap the y-axis at the 99th percentile (or 120, whichever is larger) to avoid clipping labels.
y_cap_rel <- max(120, as.numeric(quantile(df_tt$Rel_Error_pct_years, 0.99, na.rm = TRUE)))
p_violin_rel_years <- ggplot(df_tt, aes(x = Dataset, y = Rel_Error_pct_years, fill = Dataset)) +
  ...
  scale_y_continuous(limits = c(0, y_cap_rel), breaks = pretty(c(0, y_cap_rel))) +
  scale_fill_manual(values = COLORS) +
  scale_color_manual(values = COLORS) +
...
save_plot(p_violin_rel_years, "violin_rel_error_pct_years_train_test.png", w = 5, h = 5)
