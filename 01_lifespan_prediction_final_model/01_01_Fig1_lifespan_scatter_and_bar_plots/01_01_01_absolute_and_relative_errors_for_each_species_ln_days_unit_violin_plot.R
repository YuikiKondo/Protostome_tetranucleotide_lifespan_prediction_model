...
# 3) Relative Error violin plot
p_violin_rel_log_units <- ggplot(df_tt, aes(x = Dataset, y = Rel_Error_pct_log_units, fill = Dataset)) +
  geom_violin(trim = FALSE, scale = "width", alpha = 0.8, color = NA, na.rm = TRUE) +
...
save_plot(p_violin_rel_log_units, "violin_rel_error_pct_ln_days_train_test.png", w = 5, h = 5)
