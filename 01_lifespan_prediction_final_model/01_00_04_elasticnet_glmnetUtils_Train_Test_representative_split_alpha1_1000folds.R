setwd("/Users/yuikikondo/Desktop/Github_code/protostome_lifespan_prediction_model/01_lifespan_prediction_final_model")
...
data <- read.csv("filtered_species_with_zero_lifecycle_6_categories_each_region_wide_tetranuc_rates_Github_check.csv", header = TRUE)
...
data_definitive_test <- data_definitive_test %>%
  select(Organism.Name, Lifespan, log_lifespan, contains("_downstream2"), contains("_upstream2"),
         contains("_genome"), contains("_exon"), contains("_intron"))
...
# 2) Correlations (all definitive species) with p-values + global FDR (m = 256*5)
global_m <- 256 * 5  # total number of tests to correct for globally

corr_list_all <- lapply(predictor_columns, function(g) {
  x <- data_definitive$log_lifespan
  y <- suppressWarnings(as.numeric(data_definitive[[g]]))
  ii <- is.finite(x) & is.finite(y)
  n_eff <- sum(ii)
  if (n_eff >= 3 && sd(x[ii]) > 0 && sd(y[ii]) > 0) {
    ct <- suppressWarnings(cor.test(x[ii], y[ii], method = "pearson"))
    c(r = unname(ct$estimate), p = ct$p.value, n = n_eff)
  } else {
    c(r = NA_real_, p = NA_real_, n = n_eff)
  }
})

correlation_df_all <- data.frame(
  Feature_Name = predictor_columns,
  Lifespan_Feature_Correlation_All_Species = sapply(corr_list_all, function(z) z[["r"]]),
  p_value = sapply(corr_list_all, function(z) z[["p"]]),
  N = as.integer(sapply(corr_list_all, function(z) z[["n"]])),
  stringsAsFactors = FALSE
)

# Global BH–FDR (correcting as if there are 256*5 tests in total)
correlation_df_all$FDR_adjusted_p <- p.adjust(correlation_df_all$p_value, method = "BH", n = global_m)

# (Optional) also compute train-only correlations in the same style (no change to joins below)
...
corr_list_train <- lapply(predictor_columns, function(g) {
  x <- data_definitive_train_only$log_lifespan
  y <- suppressWarnings(as.numeric(data_definitive_train_only[[g]]))
  ii <- is.finite(x) & is.finite(y)
  n_eff <- sum(ii)
  if (n_eff >= 3 && sd(x[ii]) > 0 && sd(y[ii]) > 0) {
    ct <- suppressWarnings(cor.test(x[ii], y[ii], method = "pearson"))
    c(r = unname(ct$estimate))
  } else {
    c(r = NA_real_)
  }
})
correlation_df_train <- data.frame(
  Feature_Name = predictor_columns,
  Lifespan_Feature_Correlation_Train_Species = sapply(corr_list_train, function(z) z[["r"]]),
  stringsAsFactors = FALSE
)
...
# (Optional but handy) Save the full correlation table as a separate CSV
write.csv(correlation_df_all,
          "pearson_correlation_results_ALL_SPECIES_with_p_and_globalFDR.csv",
          row.names = FALSE)
