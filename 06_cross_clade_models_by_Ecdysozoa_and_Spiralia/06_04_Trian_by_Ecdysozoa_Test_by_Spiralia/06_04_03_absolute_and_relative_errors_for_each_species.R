setwd("/Users/yuikikondo/Desktop/Github_code/protostome_lifespan_prediction_model/06_cross_clade_models_by_Ecdysozoa_and_Spiralia/06_04_Trian_by_Ecdysozoa_Test_by_Spiralia")
...
# Colors for groups
COLORS <- c("Ecdysozoa (Train)" = "#33a02c",
            "Spiralia (Test)" = "#6a3d9a")
...
df_tt <- df %>%
  ...
  mutate(
    ...
    Dataset = factor(Dataset, levels = c("Ecdysozoa (Train)", "Spiralia (Test)"))
  )
...
p_scatter_all_years <- p_scatter_all_years +
  annotate("text", x = -3.8, y = 5.8, hjust = 0, vjust = 1,
           label = metrics_labels$label[metrics_labels$Dataset=="Ecdysozoa (Train)"],
           size = 4, color = COLORS["Ecdysozoa (Train)"]) +
  annotate("text", x = -3.8, y = 5.1, hjust = 0, vjust = 1,
           label = metrics_labels$label[metrics_labels$Dataset=="Spiralia (Test)"],
           size = 4, color = COLORS["Spiralia (Test)"])

save_plot(p_scatter_all_years, "plot_known_vs_pred_ln_years_train_test.png", w = 6, h = 5)
print(p_scatter_all_years)


# ===================== Correlations (Pearson's r + FDR) =====================
library(stats)
...
cor_tbl <- df_tt %>%
  group_by(Dataset) %>%
  summarise(
    cor_test = list(cor.test(Known_ln_years, Pred_ln_years, method = "pearson")),
    .groups = "drop"
  ) %>%
  rowwise() %>%
  mutate(
...
# Apply global FDR correction across both datasets
cor_tbl$FDR_p <- p.adjust(cor_tbl$p, method = "fdr")
...
p_scatter_all_years <- p_scatter_all_years +
  annotate("text", x = -3.8, y = 4.4, hjust = 0, vjust = 1,
           label = cor_tbl$label[cor_tbl$Dataset=="Ecdysozoa (Train)"],
           size = 4, color = COLORS["Ecdysozoa (Train)"]) +
  annotate("text", x = -3.8, y = 3.7, hjust = 0, vjust = 1,
           label = cor_tbl$label[cor_tbl$Dataset=="Spiralia (Test)"],
           size = 4, color = COLORS["Spiralia (Test)"])
...
p_box_pred_ln_years <- ggplot(df_tt, aes(x = Dataset, y = Pred_ln_years, fill = Dataset)) +
  ...
  scale_x_discrete(labels = c("Ecdysozoa (Train)" = "Ecdysozoa\n(Train)",
                              "Spiralia (Test)" = "Spiralia\n(Test)")) +
