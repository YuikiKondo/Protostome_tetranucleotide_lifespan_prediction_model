setwd("/Users/yuikikondo/Desktop/Github_code/protostome_lifespan_prediction_model/02_model_comparison/02_24_BUSCO_Miniprot_individual_gene_presence_absence_excluding_5spp_1000fold_seed1_alpha1")
...
data <- read.csv("filtered_species_with_zero_lifecycle_6_categories_individual_BUSCO_presence_absence_Github_check.csv", header = TRUE)
...
data_definitive_test <- data_definitive_test %>%
  select(Organism.Name, Lifespan, log_lifespan, contains("at33208"))
...
for (i in 1:999) {  # First 999 folds
    ...
    cva_model <- cva.glmnet(x_train, y_train, alpha = 0.5,
...
cv_model <- cva.glmnet(x_train, y_train, alpha = 0.5,
                       family = "gaussian", nfolds = 10, nlambda = 100)
