...
# Feature_Name: eigenvector (e.g., "eigenvector_1", "eigenvector_2")
...
data <- read.csv("phylo_eigenvectors_with_lifespan.csv", header = TRUE)
...
data_definitive_test <- data_definitive_test %>%
  select(Organism.Name, Lifespan, log_lifespan, contains("eigenvector"))
