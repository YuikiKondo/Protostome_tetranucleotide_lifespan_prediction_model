setwd("/Users/yuikikondo/Desktop/Github_code/protostome_lifespan_prediction_model/02_model_comparison/02_10_G_only_genome_size_excluding_5spp_1000folds_seed1_alpha1")
...
data <- read.csv("filtered_species_with_zero_lifecycle_6_categories_each_region_wide_tetranuc_rates_with_genome_size_Github_check.csv", header = TRUE)
...
# Remove columns
columns_to_remove <- c("Assembly.Identifier", "Phylum", "Class", "Order", 
                       "Family", "Genus")
...
data_definitive_test <- data_definitive_test %>%
  select(Organism.Name, Lifespan, log_lifespan, ln_Genome_size)
...
for (i in 1:999) {  # First 999 folds
  ...
  # ---- Fit simple linear model with single predictor ln_Genome_size ----
  # (glmnet cannot handle 1-column x, so use OLS)
  fit <- lm(log_lifespan ~ ln_Genome_size, data = fold_train_data)

  # Coefficients (named like your downstream expects)
  coefs_this_fold <- coef(fit)  # (Intercept), ln_Genome_size
  names(coefs_this_fold) <- c("(Intercept)", "ln_Genome_size")
  coef_vec_named <- coefs_this_fold

  # Predict on fold train/test
  pred_train <- predict(fit, newdata = fold_train_data)
  pred_test  <- predict(fit, newdata = fold_test_data)

  # Metrics (MAE in years computed on the original scale)
  ...
  # Store the test predictions for this fold (log-scale)
  fold_predictions[fold_test_indices, i] <- pred_test

  # ---- Predict on the final [30% test] data (never used in CV) ----
  pred_final_test <- predict(fit, newdata = test_data)  # uses ln_Genome_size in test_data
...
# ===== Step 8–10: 1000th fold (prioritize species never used as test yet) =====

# 1) Pick test indices prioritizing those never in test; cap exactly at desired size
...
# Same split ratio as other folds
test_size <- nrow(train_data) - floor(0.7 * nrow(train_data))

set.seed(1)  # reproducible
if (length(test_indices_priority) >= test_size) {
  # Too many priority indices -> sample down to exact test_size
  fold_test_indices <- sample(test_indices_priority, size = test_size, replace = FALSE)
} else {
  # Not enough priority indices -> fill the rest from remaining
  additional_needed     <- test_size - length(test_indices_priority)
  additional_test_idxs  <- sample(remaining_indices, size = additional_needed, replace = FALSE)
  fold_test_indices     <- c(test_indices_priority, additional_test_idxs)
}

# 2) Build train/test split for the 1000th fold
...
# 4) Fit OLS with the single predictor (glmnet can’t handle 1-column x)
fit <- lm(log_lifespan ~ ln_Genome_size, data = fold_train_data)

# 5) Coefficients (named consistently for downstream code)
coefs_this_fold <- coef(fit)                      # (Intercept), ln_Genome_size
names(coefs_this_fold) <- c("(Intercept)", "ln_Genome_size")
coef_vec_named <- coefs_this_fold

# 6) Predict on 1000th-fold train/test (log-scale)
pred_train <- predict(fit, newdata = fold_train_data)
pred_test  <- predict(fit, newdata = fold_test_data)

# 7) Metrics (MAE on original scale in years)
train_mae_years <- mean(abs((exp(y_train) - exp(pred_train)) / 365), na.rm = TRUE)
test_mae_years  <- mean(abs((exp(y_test)  - exp(pred_test))  / 365), na.rm = TRUE)

# 8) Store 1000th-fold test predictions
fold_predictions[fold_test_indices, 1000] <- pred_test

# 9) Final [30% holdout] predictions for this fold (never used in CV)
pred_final_test <- predict(fit, newdata = test_data)  # uses ln_Genome_size in test_data
fold_predictions_final_test_set[, 1000] <- pred_final_test

# 10) Save fold summary
