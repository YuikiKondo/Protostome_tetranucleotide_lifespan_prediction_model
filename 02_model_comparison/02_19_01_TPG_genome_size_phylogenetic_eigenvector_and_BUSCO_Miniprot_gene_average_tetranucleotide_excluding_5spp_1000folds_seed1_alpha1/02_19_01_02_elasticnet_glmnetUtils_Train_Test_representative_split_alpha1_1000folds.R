setwd("/Users/yuikikondo/Desktop/Github_code/protostome_lifespan_prediction_model/02_model_comparison/02_19_01_TPG_genome_size_phylogenetic_eigenvector_and_BUSCO_Miniprot_gene_average_tetranucleotide_excluding_5spp_1000folds_seed1_alpha1")
...
data <- read.csv("filtered_species_with_zero_lifecycle_6_categories_each_region_wide_tetranuc_rates_with_eigenvectors_and_genome_size_Github_check.csv", header = TRUE)
...
data_definitive_test <- data_definitive_test %>%
  select(Organism.Name, Lifespan, log_lifespan, ln_Genome_size, contains("_downstream2"), contains("_upstream2"), contains("_genome"), contains("_exon"), contains("_intron"), contains("eigenvector"))
